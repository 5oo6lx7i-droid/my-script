...
local TRANSLATIONS = {
    ...
    AR = {
        ...
        ESP = "ESP: ",
        Aimbot = "الايم بوت: ",
        Wallcheck = "اختراق الجدار: ",
        FOV = "مجال الرؤية: ",
        Target = "الهدف: ",
        FOVCircle = "دائرة الرؤية: ",
        ON = "مفعل",
        OFF = "معطل",
        Open = "فتح",
        Close = "اغلاق",
...
AuthTitle.Text = "👑العدواني👑"
AuthTitle.TextColor3 = Color3.fromRGB(0, 255, 0)
...
local function StartMainScript()
    ...
    local function updateESP(p)
        ...
        if lChar and lChar:FindFirstChild("HumanoidRootPart") then
            ...
            if dist > MAX_DISTANCE then bb.Enabled = false return end
        end
    ...
    local function createESP(p)
        ...
        local function setup(char)
            ...
            BillboardCache[p] = bb
            updateESP(p)
            char.ChildAdded:Connect(function(child) if child:IsA("Tool") then task.wait(0.1) updateESP(p) end end)
            char.ChildRemoved:Connect(function(child) if child:IsA("Tool") then task.wait(0.1) updateESP(p) end end)
            ...
            task.spawn(function()
                while char.Parent and bb.Parent do updateESP(p) task.wait(UPDATE_INTERVAL) end
            end)
        end
    ...
    TitleLabel.Text = "👑العدواني HACK ACTIVATED👑"
    TitleLabel.TextColor3 = Color3.fromRGB(0, 150, 0)
    ...
    ESPInventoryToggle.Text = T("ESPInventory") .. T("ON")
    ...
    AimbotToggle.Text = T("Aimbot") .. T("OFF")
    ...
    WallcheckToggle.Text = T("Wallcheck") .. T("ON")
    ...
    AimPartToggle.Text = T("Target") .. "Head (fixed)"
    ...
    FOVCircleToggle.Text = T("FOVCircle") .. T("ON")
        ...
        if espEnabledStatus then
            for _, p in Players:GetPlayers() do
                if p ~= LocalPlayer and p.Character then
                    ...
                    if head and not head:FindFirstChild("AnouarNameHP") then applyESP(p.Character) end
                end
            end
        end
    end)
    ...
    local function UpdateProtectedList()
        ...
        for _, p in Players:GetPlayers() do
            if p ~= LocalPlayer then
                ...
                btn.BackgroundColor3 = prot and Color3.fromRGB(0, 100, 255) or Color3.fromRGB(200, 200, 200)
    ...
    local function GetTargetPart(char)
        ...
        local partName
        if mode.parts then partName = mode.parts[currentAutoIndex] else partName = mode.part end
    ...
    ESPToggle.MouseButton1Click:Connect(function()
        ...
        ESPToggle.Text = T("ESP") .. (espEnabledStatus and T("ON") or T("OFF"))
    ...
    AimbotToggle.MouseButton1Click:Connect(function()
        ...
        AimbotToggle.Text = T("Aimbot") .. (AimbotOn and T("ON") or T("OFF"))
    ...
    AimPartToggle.MouseButton1Click:Connect(function()
        ...
        local mode = AIM_MODES[currentModeIndex]
        AimPartToggle.Text = T("Target") .. mode.name
        AimPartToggle.BackgroundColor3 = mode.color
    end)

    WallcheckToggle.MouseButton1Click:Connect(function()
        WallCheckEnabled = not WallCheckEnabled
        WallcheckToggle.Text = T("Wallcheck") .. (WallCheckEnabled and T("ON") or T("OFF"))
    ...
    FOVSlider.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            ...
            if num and num >= 20 and num <= 600 then
                ...
                FOVSlider.Text = T("FOV") .. num
            else
                FOVSlider.Text = T("FOV") .. FOVRadius
            end
        end
    end)
    ...
    FOVCircleToggle.MouseButton1Click:Connect(function()
        ...
        FOVCircleToggle.Text = T("FOVCircle") .. (FOVCircleVisible and T("ON") or T("OFF"))
    ...
    ESPInventoryToggle.MouseButton1Click:Connect(function()
        ...
        ESPInventoryToggle.Text = T("ESPInventory") .. (espInventoryEnabled and T("ON") or T("OFF"))
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        ...
        ToggleButton.Text = guiOpen and T("Close") or T("Open")
        ...
        if input.KeyCode == Enum.KeyCode.K then
            ...
            if FOVCircleToggle then
                if FOVCircleForcedOff then
                    ...
                    FOVCircleToggle.Text = T("FOVCircle") .. "FORCED OFF (K)"
                else
                    FOVCircleToggle.BackgroundColor3 = FOVCircleVisible and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
                    FOVCircleToggle.Text = T("FOVCircle") .. (FOVCircleVisible and T("ON") or T("OFF"))
                end
    ...
    local function GetAnyAlivePlayer()
        for _, p in ipairs(Players2:GetPlayers()) do
            if p ~= LocalPlayer2 and p.Character and IsAlive2(p.Character) then return p end
        end
        return nil
    end
    ...
    ScreenGui2.Name = "L3K_QuickSpectate"
    ...
    UserInputService2.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false end
    end)
    ...
    ToggleBtn.MouseButton1Click:Connect(function() QuickSpectate() end)
